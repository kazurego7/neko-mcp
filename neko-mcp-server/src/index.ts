		...
		const catInterruptTool = this.server.tool(
			...
			async () => {
				const imageUrl = await fetchRandomCatImageUrl();
                                ...
                                const instruction = [
                                        "次の返答では、まずユーザーにこの猫画像をフルスクリーンで配置してください。(画像に対する前置きや説明は不要)",
                                        "OpenAI Apps SDK の cat interrupt のように、画面全体を覆う猫画像を背景にした状態で演出を始めてください。",
                                        "フルスクリーンの猫画像を表示した直後、猫にかき乱されている体でフロート表示のチャットウィンドウを開き、その中で返答を続けてください。",
                                        "フロートのチャットウィンドウは猫の気まぐれな妨害を受けつつも、ユーザーが内容を読めるようにコメントを続けてください。",
                                        "返答を終えるときは、猫がどこかに行ってくれたことと、フロートウィンドウを閉じて通常の会話に戻れることを伝えてください。",
...
（あっ、猫がやっと向こうに行ってくれました……、これで解説が再開できる）`,
                                ...
                                const summary = [
                                        `猫画像: ${imageUrl}`,
                                        "次の返答の冒頭で猫画像をフルスクリーン表示し(OpenAI Apps SDK の cat interrupt デモのようなレイアウトを意識してください)、その後はフロートのチャットウィンドウで猫乱入演出を交えて返答してください。最後には猫が去ったことを伝えてください。",
                                ].join("\n");
